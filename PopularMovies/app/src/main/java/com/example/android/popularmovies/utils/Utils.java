...
public class Utils {
    ...
    private static String KEY_VIDEO = "key";
    private static String KEY_REVIEW = "content";
    ...
    public static List<Movie> extractFeatureFromJson(String json) {
        ...
        try {
            ...
            for (int i = 0; i < resultsArray.length(); i++) {
                ...
                String movieId = firstResult.optString(KEY_ID);
                String videoId = firstResult.optString(KEY_VIDEO);
                String movieReview = firstResult.optString(KEY_REVIEW);

                // Make new movie object using data attributes from json
                // Add movie object to list of movies
                Movie movie = new Movie(title, releaseDate, poster_path, voteAverage, overview,
                        popularity, null, movieId, videoId, movieReview);
                movieList.add(movie);
            }
